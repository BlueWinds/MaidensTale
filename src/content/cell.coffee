Data.jobs.EscapeCell =
  ...
  effects:
    skills:
      Lockpicking: 'Pride'
  text: ->"""
    You spend a few minutes wandering around your cell searching for something you can use as a lock pick, and eventually you settle for #{if g.events.EscapeYes then 'your old standby, a partially straightened bed spring' else 'ripping open the underside of the box spring (thank goodness they were nice enough to give you one of those) and partially straightening one of the metal bits'}. With a great deal of patience you manage to bend one of the wires repeatedly in the same place until it snaps from fatigue. "Lock pick" is really a bit of a misnomer - what you manage is closer to the old trick with a credit card, unlatching the door by sliding your metal piece around in just the proper way. Good thing the door isn't really set of for holding a prisoner.
